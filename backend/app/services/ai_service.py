...
def _generate_with_groq(api_key: str, content: str, count: int) -> list:
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Generate {count} quiz questions based on this content:\n\n{content}"},
        ],
        max_tokens=4096,
        temperature=0.7,
    )
...
def _generate_with_gemini(api_key: str, content: str, count: int) -> list:
    ...
    response = model.generate_content(
        f"Generate {count} quiz questions based on this content:\n\n{content}"
    )
...
def generate_questions(
    ...
    if groq_api_key:
        try:
            questions = _generate_with_groq(groq_api_key, content, count)
    ...
    if gemini_api_key:
        try:
            questions = _generate_with_gemini(gemini_api_key, content, count)
